...
import com.skilrock.retailapp.ui.fragments.rms.PayoutFragment;
...
public interface Utils {
    ...
    static UrlOlaBean getOlaUrlDetails(HomeDataBean.ResponseData.ModuleBeanList.MenuBeanList menuBean, Context context, String tag) {
        ...
        try {
            ...
            if (BuildConfig.FLAVOR.equalsIgnoreCase("qA_OLA_MYANMAR") ||
                    BuildConfig.FLAVOR.equalsIgnoreCase("uAT_OLA_MYANMAR") ||
                    BuildConfig.FLAVOR.equalsIgnoreCase("pROD_OLA_MYANMAR") ||
                    BuildConfig.FLAVOR.equalsIgnoreCase("uAT_CAMRN") ||
                    BuildConfig.FLAVOR.equalsIgnoreCase("pROD_CAMRN") ||
                    BuildConfig.FLAVOR.equalsIgnoreCase("qA_CAMRN") ||
                    BuildConfig.FLAVOR.equalsIgnoreCase("qA_PAYPR_BUSINESS") ||
                    BuildConfig.FLAVOR.equalsIgnoreCase("uAT_PAYPR_BUSINESS") ||
                    BuildConfig.FLAVOR.equalsIgnoreCase("pROD_PAYPR_BUSINESS")) {
    ...
    static UrlOrgBean getDoPayAmountDetailUrl(HomeDataBean.ResponseData.ModuleBeanList.MenuBeanList menuBean,  Context context, String tag) {
        UrlOrgBean urlOrgBean = new UrlOrgBean();
        try {
            JSONObject jsonObject = new JSONObject(menuBean.getApiDetails());
            String url = menuBean.getBasePath() + jsonObject.getJSONObject(tag).getString("url");
            urlOrgBean.setUrl(url);
            JSONObject headerObject = jsonObject.getJSONObject(tag).getJSONObject("headers");
            urlOrgBean.setBalance(headerObject.getString("balance"));
            urlOrgBean.setAccept(headerObject.getString("Accept"));
            return urlOrgBean;
        } catch (JSONException e) {
            Toast.makeText(context, context.getString(R.string.url_problem), Toast.LENGTH_SHORT).show();
            e.printStackTrace();
...
*/
    ...
    static Drawable getLogo(Context context, String Key) {
        ...
        map.put(AppConstants.PAYPR_BUSINESS, context.getResources().getDrawable(R.drawable.paypr_business));
    ...
    static String getAppVersionName(Context context) {
        try {
            ...
            String internal_version = BuildConfig.INTERNAL_VERSION == null ? " ":"  "+"("+BuildConfig.INTERNAL_VERSION+")";
            return String.format("%s %s%s", context.getString(R.string.version), pInfo.versionName, internal_version);
        } catch (PackageManager.NameNotFoundException e) {
            e.printStackTrace();
            return "";
        }
...
*/
    ...
    static String getDeviceSerialNumber() {
        ...
        if (serialNumber != null && serialNumber.equalsIgnoreCase("unknown"))
            serialNumber = "NA";
    ...
    static String getAmountWithCurrency(String strAmount, Context context) {
        ...
        try {
            formattedAmount = getBalanceToView(Double.parseDouble(strAmount),
                    getCurrencyFormatter(SharedPrefUtil.getLanguage(context)),
                    getAmountFormatter(SharedPrefUtil.getLanguage(context)),
                    Integer.parseInt(ConfigData.getInstance().getConfigData().getALLOWEDDECIMALSIZE()))
                    + " " + getDefaultCurrency(SharedPrefUtil.getLanguage(context));
        } catch (Exception e) {
            e.printStackTrace();
...
//If the id is -1, that means the record was found in the APN table before insertion, thus, no action required
    ...
    static String getIp() {
        String getIp = null;
        try {
            getIp = new NetTask().execute(BuildConfig.URL).get();
        } catch (Exception e1) {
            e1.printStackTrace();
        }
        return getIp;
